# Isaac Sim Action Graph Component

## Component Overview

This component provides a sophisticated OmniGraph-based simulation pipeline that processes joint states, end-effector poses, and camera data through ROS2 topics. The action graph implements a complete simulation pipeline stage configuration with proper data flow between nodes.

## Core Features

### 1. Node Architecture
- **Timing and Context Nodes**: Core simulation tick and ROS2 context management
- **Joint State Management**: Publish and subscribe to robot joint states
- **Robot Control**: Articulation controller for joint position/velocity commands
- **End-Effector Reading**: Extract translation and rotation from USD prims
- **Vector Processing**: Break down pose data into individual components
- **ROS2 Integration**: Bridge Isaac Sim and ROS2 communication
- **Camera Rendering**: Real-time camera image generation and publishing

### 2. Data Flow Pipeline
- Simulation tick drives joint state updates
- Joint state commands feed robot articulation controller
- End-effector poses are extracted and published via ROS2
- Gripper width calculated from finger positions and published
- Camera renders RGB images at simulation rate
- All components properly timestamped with simulation timecodes
...
### Updated launch.py Integration

The action graph component should be integrated into the main launcher script:

```python
# In IsaacSimLauncher.launch_isaac_sim() method:

# After scene loading and before simulation loop
print("Creating simulation action graph...")
action_graph = IsaacSimActionGraph(self.task_name, str(usd_path))
action_graph.create_action_graph()
action_graph.set_pipeline_stage()
action_graph.start()

# Main simulation loop (modified to work with action graph)
while simulation_app.is_running():
    simulation_app.update()

# Cleanup on exit
print("Shutting down Isaac Sim...")
if 'action_graph' in locals():
    action_graph.stop()
    action_graph.cleanup()
...
- Separate action graph component for clear separation of concerns
- Easy to extend with additional nodes and connections
- Configurable through task registries

### 2. ROS2 Integration
- Automatic joint state publishing and command subscription
- End-effector pose publishing with proper timestamps
- Gripper width calculation and publishing
- Camera image generation and ROS2 topic publishing

### 3. Real-time Processing
- Simulation tick-driven processing at appropriate rates
- Proper timecode synchronization between components
- Efficient data flow through OmniGraph optimization

### 4. Extensibility
- Easy to add new ROS2 topics or processing nodes
- Configurable camera parameters and rendering settings
- Support for different robot configurations through task registries

## Usage in Task Registries

Each task registry should specify any action graph-specific configurations:

```python
# In task registry classes:
ACTION_GRAPH_CONFIG = {
    "joint_state_topic": "/joint_states",
    "joint_command_topic": "/joint_command",
    "eeff_pose_topic": "/eeff_states",
    "gripper_width_topic": "/gripper_width",
    "camera_topic": "/camera/rgb/image_raw",
    "robot_prim_path": "/World/panda",
    "eeff_prim_path": "/World/panda/panda_link7",
    "finger_left_path": "/World/panda/panda_leftfinger",
    "finger_right_path": "/World/panda/panda_rightfinger",
    "camera_prim_path": "/World/panda/panda_link7/gopro_link/camera",
}
```

This action graph component provides a sophisticated, production-ready simulation pipeline that fully integrates Isaac Sim with ROS2 for robotics applications.