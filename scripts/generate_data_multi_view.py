...
def save_multi_episode_dataset(output_path: str, episodes: list[dict]) -> None:
    """
    Save multi-episode dataset with multi-view images and extrinsics.
    """
    ...
    for key in camera_ext_keys:
        ext_data = np.concatenate([ep[key] for ep in episodes], axis=0).astype(
            np.float32
        )
...
def step_world_and_record(
    ...
    """
    Step world and record multi-view images & camera extrinsics.

    Args:
        cameras: dict of camera_key -> Camera object
        rgb_dict_list: list to append {cam_key: rgb_image} dicts
        extrinsics_dict_list: list to append {cam_key: 4x4 matrix} dicts
    """
    ...
    if frame_dict:
        ...
        extrinsics_dict_list.append(ext_dict)
...
def main():
    ...
    for episode_idx in episodes_to_run:
        ...
        demo_start_list = np.repeat(
            episode_start_pose[None, :], len(rgb_dict_list), axis=0
        )
