...
class PipelineExecutor:
    ...
    def _log_config_diff(self, stage_name: str, inherited_config: Dict[str, Any], 
                        stage_config: Dict[str, Any], final_config: Dict[str, Any]) -> None:
        ...
        """
        self.logger.info(f"Configuration for stage '{stage_name}':")
        
        if not inherited_config:
            self.logger.info("  No inherited configuration (first stage or inherit_config=false)")
        else:
            ...
            if overridden_keys:
                self.logger.info(f"  Overridden keys: {list(overridden_keys)}")
            
            new_keys = local_keys - inherited_keys
            if new_keys:
                self.logger.info(f"  New keys: {list(new_keys)}")
            
            self.logger.info(f"Inherited {len(inherited_config)} keys, added {len(new_keys)} keys, overrode {len(overridden_keys)} keys")
    ...
    def load_service(self, stage_name: str, propagated_config: Dict[str, Any] = None) -> BaseService:
        ...
        try:
            ...
            self.logger.info(f"Loaded service for stage '{stage_name}': {class_path}")
            return service_instance
            
        except Exception as e:
            self.logger.error(f"Failed to load service for stage '{stage_name}': {e}")
            raise
    ...
    def execute_stage(self, stage_name: str, propagated_config: Dict[str, Any] = None, *args, **kwargs):
        ...
        try:
            ...
            self.logger.error(f"Failed to execute stage '{stage_name}': {e}")
            raise
    ...
    def execute_all(self, *args, **kwargs) -> dict:
        ...
        self.logger.info("Configuration propagation enabled - configs from previous stages will be passed forward")
        
        for i, stage_name in enumerate(stages, 1):
            stage_config = self.config[stage_name]
            inherit_config = stage_config.get('inherit_config', True)
            
            self.logger.info(f"Stage {i}/{len(stages)}: {stage_name} (inherit_config: {inherit_config})")
            ...
            try:
                ...
                if inherit_config:
                    ...
                    self.logger.info(f"  Updated propagated configuration with {len(stage_effective_config)} keys")
                else:
                    self.logger.info("  Skipped configuration propagation for this stage")
                    
            except Exception as e:
                self.logger.error(f"Pipeline failed at stage '{stage_name}': {e}")
                raise
    ...
    def validate_stages(self) -> dict:
        ...
        for stage_name in self.list_stages():
            try:
                ...
                self.logger.warning(f"Validation failed for stage '{stage_name}': {e}")
