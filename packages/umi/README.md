...
Known issue: The policy doesn't work well under direct sunlight, since the dataset was collected during a rainy week at Stanford.
...
The ArUco Detection Viewer is a visualization tool for exploring ArUco marker detection results generated by the UMI pipeline. It allows users to inspect detected markers, their positions, orientations, and track detection quality across video frames.

#### Installation

The viewer is included in the voilab package. Ensure you have the required dependencies:

```bash
# The viewer depends on opencv-contrib-python which is included in umi dependencies
# Additional voilab dependencies can be installed with:
make deps  # from the voilab directory
```
...
# Interactive frame-by-frame viewer with default size (8x6 inches)
aruco_detection_viewer.show("/path/to/directory")

# Viewer with custom size
aruco_detection_viewer.show("/path/to/directory", figsize=(6, 4), dpi=100)

# View multiple frames in a grid
aruco_detection_viewer.show_batch("/path/to/directory", [0, 10, 20, 30])

# Batch view with custom subplot sizes
aruco_detection_viewer.show_batch("/path/to/directory", [0, 10, 20, 30], subplot_size=(2.5, 2))
```

##### Jupyter Notebook

Open `nbs/aruco_detection_viewer.ipynb` in Jupyter and use the interactive widgets for directory selection and size control.

#### Requirements

The selected directory must contain:
- `raw_video.mp4` - Input video file
- `tag_detection.pkl` - Detection results from UMI pipeline

#### Features

- Frame-by-frame navigation with slider control
- Configurable display size (figure dimensions and DPI)
- Marker visualization with IDs and coordinate axes
- Pose information display (position and orientation vectors)
- Detection statistics overview
- Batch viewing capability for multiple frames

#### File Structure

```
voilab/
├── src/voilab/utils/aruco_detection_loader.py    # Data loading utility
├── src/voilab/applications/aruco_detection_viewer.py  # Main application
└── nbs/aruco_detection_viewer.ipynb               # Notebook interface
```
